 ...
 * 스킨이 너무 크고 흐릿한 경우, Rainmeter가 설치된 폴더(Program Files/Rainmeter)에서 Rainmeter.exe 우클릭 후 호환성, 높은 DPI 설정 변경을 누른 뒤, 높은 DPI 조정 동작을 재정의합니다. 를 누르면 됩니다. Rainmeter는 HiDPI를 지원하지 않으며, 이를 해결하기 위해 기존에는 플러그인을 사용했으나, 외부 코드 사용을 원치 않아 제거했습니다. 수동으로 설정해야 합니다.
