 ...
 * 스킨 크기가 너무 크고 흐릿한 경우에는 Rainmeter가 설치된 폴더에 들어가(Program Files/Rainmeter) Rainmeter.exe 우클릭 후 호환성 설정에서 높은 DPI 설정 변경을 누른 뒤, 높은 DPI 조정 동작을 재정의합니다. 를 누르면 됩니다. Rainmeter는 HiDPI를 지원하지 않아 기존에는 별도의 플러그인을 사용했으나, 외부 코드를 사용하는 것을 원치 않아 제거했습니다. 수동으로 설정해주셔야 합니다.
